...
class DCGAN_D(nn.Module):
    ...
    def __init__(self, isize, nc, ndf, ngpu, n_extra_layers=0):
        ...
        main.add_module(f'final{cndf}-1conv', nn.Conv2d(cndf, 1, 4, 1, 0, bias=False))
        self.main = main
...
class DCGAN_G(nn.Module):
    ...
    def forward(self, input):
        ...
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            gpu_ids = range(self.ngpu)
        temp = (nn.parallel.data_parallel(self.main, input, gpu_ids)+1)/2
        return tf.image.resize_images(temp, [256, 256])
